...
class StoryFetcher:
    ...
    def __get_comments(self, story):
        futures = [self.hn.get_item(comment_id) for comment_id in story.kids]
        comments = []
...
class HackernewsService:
    ...
    def get_all_stories(self, callback=None):
        ...
        return self.session.get('{}{}'.format(self.endpoint, 'topstories.json'), background_callback=callback)

    def get_item(self, story_id, callback=None):
        session = FuturesSession(max_workers=10)
        return self.session.get('{}{}{}{}'.format(self.endpoint, 'item/', str(story_id), '.json'), background_callback=callback)
